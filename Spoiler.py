...
class Spoiler(object):
    ...
    def parse_data(self):
        ...
        for world in self.worlds:
            ...
            self.entrances[world.id] = OrderedDict([(world.entrances[entrance]["name"], world.entrances[world.entrances[entrance]["actual"]]["name"]) for entrance in world.entrances])
    ...
    def spoiler_output(self):
        ...
        for world in self.worlds:
            ...
            output += header_world_string.format(header="Entrances", world=world.id+1)
            output += '\n'.join(['{:{width}} {}'.format(location_string.format(location=location, world=world.id+1), entrance_string.format(entrance=entrance), width=location_padding) for (location, entrance) in self.entrances[world.id].items()])
